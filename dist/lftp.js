...
class LFTP {
  constructor(opts) {
    ...
    this.spawn = _child_process2.default.spawn;
    this.spawnOptions = {};
  ...
  validateOptions(opts) {
    ...
    if (opts.hasOwnProperty('protocol') && !(0, _includes3.default)(acceptableProtocols, opts.protocol)) {
  ...
  escapeShell(cmd) {
    if ((0, _typeDetect2.default)(cmd) !== 'string') {
      return '';
    }
  ...
  baseCmd() {
    ...
    cmd.push(`set net:reconnect-interval-multiplier ${this.options.retryIntervalMultiplier}`);

    cmd.push(`open -u "${this._escapeShell(this.options.username)}","${this._escapeShell(this.options.password)}" "${this.options.host}"`);
  ...
  exec(extraCmds) {
    ...
    if ((0, _typeDetect2.default)(extraCmds) === 'string') {
      cmd.push(extraCmds);
  ...
  alias(name, value) {
    ...
    return this.raw(`alias ${this._escapeShell(name)} ${this._escapeShell(value)}`);
  }
  ...
  cache(subCMD, opts = {}) {
    ...
    switch (subCMD) {
      ...
      case 'expire':
        ...
        if (opts.hasOwnProperty('n') && opts.hasOwnProperty('unit') && (0, _includes3.default)(acceptableUnits, opts.unit)) {
  ...
  chmod(mode, ...files) {
    ...
    return this.raw(`chmod ${mode} ${(0, _map3.default)(files, this._escapeShell.bind(this)).join(' ')}`);
  }
  ...
  cls(opts = {}) {
    ...
    const singleColumn = opts.hasOwnProperty('singleColumn') && opts.singleColumn;
  ...
  edit(file, opts = {}) {
    ...
    if (!file && (0, _typeDetect2.default)(file) !== 'string') {
      return this;
    }
  ...
  jobs(opts = {}) {
    ...
    if (opts.hasOwnProperty('verbosity') && (0, _inRange3.default)(opts.verbosity, 1, 4)) {
      cmd.push(`-${(0, _times3.default)(opts.verbosity, () => {
        return 'v';
      }).join('')}`);
    }
  ...
  rm(...files) {
    ...
    return this.raw(`rm ${(0, _map3.default)(files, this._escapeShell.bind(this)).join(' ')}`);
  }
  ...
  rmdir(...directories) {
    ...
    return this.raw(`rmdir ${(0, _map3.default)(directories, this._escapeShell.bind(this)).join(' ')}`);
  }
